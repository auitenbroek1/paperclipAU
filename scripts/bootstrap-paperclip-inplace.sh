#!/usr/bin/env bash
# ============================================================================
# bootstrap-paperclip-inplace.sh — Bootstrap Paperclip in an EXISTING directory
# ============================================================================
#
# For use in GitHub Codespaces, pre-cloned repos, or any existing directory.
# Unlike bootstrap-paperclip.sh, this does NOT create a new directory or
# run git init — it works in the current working directory.
#
# Usage:
#   cd my-existing-repo
#   bash bootstrap-paperclip-inplace.sh [--dry-run]
#
...
info "Working in-place in: $(pwd)"
info "Project name (from directory): $PROJECT_NAME"
...
# --- Step 1: Verify current directory is suitable ---------------------------
step 1 "Verifying current directory"

if [[ -f "package.json" ]]; then
  warn "package.json already exists — it will be OVERWRITTEN"
  if ! $DRY_RUN; then
...
if git rev-parse --is-inside-work-tree &>/dev/null; then
  ok "Already a git repo — skipping git init"
else
...
step 2 "Creating root configuration files"
...
# --- Step 3: Monorepo package scaffolds -------------------------------------
step 3 "Scaffolding monorepo packages"
...
# --- Step 4: Server package -------------------------------------------------
step 4 "Scaffolding Express 5 server"
...
# --- Step 5: UI package -----------------------------------------------------
step 5 "Scaffolding React 19 + Vite frontend"
...
# --- Step 6: Root tsconfig with references ----------------------------------
step 6 "Creating root tsconfig project references"
...
step 7 "Installing npm dependencies (this may take a few minutes)"

run pnpm install
ok "Dependencies installed"

# --- Step 8: RuVector ONNX models -------------------------------------------
step 8 "Downloading RuVector ONNX models (all-MiniLM-L6-v2)"
...
# --- Step 9: RuVector bootstrap script -------------------------------------
step 9 "Creating RuVector bootstrap script"
...
# --- Step 10: Ruflo (claude-flow) -------------------------------------------
step 10 "Installing Ruflo (claude-flow) orchestration"
...
# --- Step 11: Superpowers ---------------------------------------------------
step 11 "Installing Superpowers skill system"
...
# --- Step 12: Paperclip Memory + lessons ------------------------------------
step 12 "Setting up Paperclip Memory and lessons file"
...
# --- Step 13: Claude Code hooks (settings.json) -----------------------------
step 13 "Configuring Claude Code hooks"
...
# --- Done (NO git commit — user manages their own repo) --------------------
printf "\n${GREEN}${BOLD}━━━ Bootstrap complete (in-place)! ━━━${NC}\n\n"

if $DRY_RUN; then
  printf "(Dry run — no files were created)\n\n"
fi

printf "The Paperclip stack has been bootstrapped in: ${CYAN}$(pwd)${NC}\n\n"

printf "Next steps:\n"
printf "  ${CYAN}git add -A && git commit -m 'Bootstrap Paperclip stack'${NC}\n"
